...
static Tk_CustomOption stylesOption = {
    Rbc_StringToStyles, Rbc_StylesToString, (ClientData)sizeof(LinePenStyle)
};
...
static Tk_ConfigSpec linePenConfigSpecs[] = {
    ...
    {TK_CONFIG_CUSTOM, "-outlinewidth", "outlineWidth", "OutlineWidth",
     ...
     TK_CONFIG_DONT_SET_DEFAULT | ALL_PENS, &rbcDistanceOption},
    {TK_CONFIG_CUSTOM, "-pixels", "pixels", "Pixels",
    ...
    Rbc_Chain *palette;
{
    ...
    for (linkPtr = Rbc_ChainFirstLink(palette); linkPtr != NULL;
    ...
    double tolerance;
{
    ...
    n = Rbc_SimplifyLine(mapPtr->screenPts, 0, mapPtr->nScreenPts - 1,
                         tolerance, simple);
    for (i = 0; i < n; i++) {
        k = simple[i];
    ...
    MapInfo *mapPtr;
{
    ...
    result = FALSE;
    if (linePtr->smooth == PEN_SMOOTH_NATURAL) {
        result = Rbc_NaturalParametricSpline(origPts, nOrigPts, &exts, FALSE,
                                             intpPts, nIntpPts);
    } else if (linePtr->smooth == PEN_SMOOTH_CATROM) {
        result = Rbc_CatromParametricSpline(origPts, nOrigPts, intpPts,
                                            nIntpPts);
    }
    if (!result) {
        /* The spline interpolation failed.  We'll fallback to the
    ...
    PenStyle **dataToStyle;
{
    ...
    if (linePtr->nStrips > 0) {
        ...
        for (linkPtr = Rbc_ChainFirstLink(linePtr->palette); linkPtr != NULL;
                linkPtr = Rbc_ChainNextLink(linkPtr)) {
            stylePtr = Rbc_ChainGetValue(linkPtr);
    ...
    if (linePtr->xErrorBarCnt > 0) {
        ...
        for (linkPtr = Rbc_ChainFirstLink(linePtr->palette);
    ...
    Element *elemPtr; /* Element component record */
{
     ...
     */
    if ((nPoints > 1) && ((graphPtr->classUid == rbcStripElementUid) ||
    ...
    int *pointToData;
{
    ...
    for (pointPtr = symbolPts, endPtr = symbolPts + nSymbolPts;
        ...
        Rbc_DrawText(graphPtr->tkwin, drawable, string, &(penPtr->valueStyle),
    ...
    Element *elemPtr; /* Element to be drawn */
{
    ...
    if (linePtr->nActiveIndices > 0) {
        ...
        if (penPtr->traceWidth > 0) {
            if (linePtr->nStrips > 0) {
                Rbc_Draw2DSegments(graphPtr->display, drawable,
    ...
    Element *elemPtr; /* Element to be drawn */
{
    ...
    if (linePtr->fillPts != NULL) {
        ...
        if (linePtr->fillTile != NULL) {
            ...
            Rbc_TilePolygon(graphPtr->tkwin, drawable, linePtr->fillTile,
    ...
    for (linkPtr = Rbc_ChainFirstLink(linePtr->palette); linkPtr != NULL;
        ...
        if ((stylePtr->xErrorBarCnt > 0) && (penPtr->errorBarShow & SHOW_X)) {
            Rbc_Draw2DSegments(graphPtr->display, drawable, penPtr->errorBarGC,
    ...
    int size;
{
    ...
    if (penPtr->symbol.type == SYMBOL_NONE) {
        Rbc_LineAttributesToPostScript(psToken, defaultColor,
    ...
    switch (penPtr->symbol.type) {
        ...
        case SYMBOL_BITMAP: {
                ...
                if ((penPtr->symbol.mask != None) && (fillColor != NULL)) {
                    ...
                    Rbc_BitmapToPostScript(psToken, graphPtr->display,
    ...
    int *pointToData;
{
    ...
    for (pointPtr = symbolPts, endPtr = symbolPts + nSymbolPts;
        ...
        Rbc_TextToPostScript(psToken, string, &(penPtr->valueStyle),
    ...
    Element *elemPtr;
{
    ...
    if (linePtr->nActiveIndices > 0) {
        ...
        if (penPtr->traceWidth > 0) {
            if (linePtr->nStrips > 0) {
                SetLineAttributes(psToken, penPtr);
                Rbc_2DSegmentsToPostScript(psToken, linePtr->strips,
                                           linePtr->nStrips);
            }
    ...
    Element *elemPtr;
{
    ...
    if (linePtr->fillPts != NULL) {
        ...
        if (linePtr->fillTile != NULL) {
            ...
            Rbc_StippleToPostScript(psToken, graphPtr->display,
                                    linePtr->fillStipple);
        } else {
            Rbc_AppendToPostScript(psToken, "Fill\n", (char *)NULL);
        }
    ...
    if (linePtr->nStrips > 0) {
        for (linkPtr = Rbc_ChainFirstLink(linePtr->palette); linkPtr != NULL;
            ...
            if ((stylePtr->nStrips > 0) && (penPtr->traceWidth > 0)) {
                SetLineAttributes(psToken, penPtr);
                Rbc_2DSegmentsToPostScript(psToken, stylePtr->strips,
                                           stylePtr->nStrips);
            }
        }
    } else if ((Rbc_ChainGetLength(linePtr->traces) > 0) &&
    ...
    for (linkPtr = Rbc_ChainFirstLink(linePtr->palette); linkPtr != NULL;
        ...
        if ((stylePtr->xErrorBarCnt > 0) && (penPtr->errorBarShow & SHOW_X)) {
            Rbc_LineAttributesToPostScript(psToken, colorPtr,
                                           penPtr->errorBarLineWidth, NULL, CapButt, JoinMiter);
            Rbc_2DSegmentsToPostScript(psToken, stylePtr->xErrorBars,
                                       stylePtr->xErrorBarCnt);
        }
        if ((stylePtr->yErrorBarCnt > 0) && (penPtr->errorBarShow & SHOW_Y)) {
            Rbc_LineAttributesToPostScript(psToken, colorPtr,
                                           penPtr->errorBarLineWidth, NULL, CapButt, JoinMiter);
            Rbc_2DSegmentsToPostScript(psToken, stylePtr->yErrorBars,
