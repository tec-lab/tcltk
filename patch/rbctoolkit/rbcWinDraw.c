 ...
 * for this is awful
 * was made especially for rbc, graph4 demo now runs 4x faster
 *
 */
    ...
    TextLayout *textPtr;
{
    ...
    Rbc_TranslateAnchor(x, y, bbWidth, bbHeight, tsPtr->anchor, &x, &y);
    ...
    int destY;
{
    ...
    srcBits = Rbc_GetBitmapData(display, srcBitmap, width, height,
                                &bytesPerRow);
    if (srcBits == NULL) {
        return;
    }
    ...
    if (maskBitmap != None) {
        ...
        maskBits = Rbc_GetBitmapData(display, maskBitmap, width, height,
                                     &bytesPerRow);
        bytesPerRow = ((width + 31) & ~31) / 8;
 ...
 *
 *      This differs from Tk's XFillPolygon in that it works
 *      around deficencies in Windows 95/98:
